...
class YOLO():
    ...
    def forward(self, inputs, isTrain=True):
        ...
        # y1 特征
        inter1, net = module.yolo_block(route_3, 512, isTrain=isTrain)
        feature_y1 = module.CBL(net, 3*(5+self.class_num), 1, bn=False, relu=False, isTrain=isTrain) # 不激活
         
        inter1 = module.CBL(inter1, 256, 1, isTrain=isTrain)
        inter1 = module.yolo_upsample(inter1, tf.shape(route_2))
        concat1 = module.yolo_concat(inter1, route_2)
        # 得到V2的特征
        inter2, net = module.yolo_block(concat1, 256, isTrain=isTrain)
        feature_y2 = module.CBL(net, 3*(5 + self.class_num), 1, bn=False, relu=False, isTrain=isTrain) # 不激活
        
        inter2 = module.CBL(inter2, 128, 1, isTrain=isTrain)
        inter2 = module.yolo_upsample(inter2, tf.shape(route_1))
        concat2 = module.yolo_concat(inter2, route_1)
        # 得到V3的特征
        _, net = module.yolo_block(concat2, 128, isTrain=isTrain)
        feature_y3 = module.CBL(net, 3*(5 + self.class_num),1, bn=False, relu=False, isTrain=isTrain) # 不激活
    ...
    # 计算损失
    def __compute_loss(self, xy, wh, conf, prob, yi_true, low_iou_mask):
        '''
    ...
    def get_loss(self, feature_y1, feature_y2, feature_y3, y1_true, y2_true, y3_true, use_iou=True, ignore_thresh=0.5):
        ...
        loss_y1 = self.__compute_loss(xy, wh, conf, prob, y1_true, low_iou_mask_y1)
        ...
        loss_y2 = self.__compute_loss(xy, wh, conf, prob, y2_true, low_iou_mask_y2)
        ...
        loss_y3 = self.__compute_loss(xy, wh, conf, prob, y3_true, low_iou_mask_y3)
